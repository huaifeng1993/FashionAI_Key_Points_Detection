...
下面这段代码是同时对5个结果合并
'''
# blouse_result=pd.read_csv('../data_b/tem_resutl/blouse_result_b.csv')
# dress_resutl=pd.read_csv('../data_b/tem_resutl/dress_result_b.csv')
# outwear_resutl=pd.read_csv('../data_b/tem_resutl/outwear_result_b.csv')
# skirt_resutl=pd.read_csv('../data_b/tem_resutl/skirt_result_b.csv')
# trousers_resutl=pd.read_csv('../data_b/tem_resutl/trousers_result_b.csv')
# result=pd.concat([blouse_result,dress_resutl,outwear_resutl,skirt_resutl,trousers_resutl])
# print(result.shape[0])
# test=pd.read_csv('../data_b/test.csv')
# print(test.shape[0])

# asser result.shpae[0] == test.shape[0] #做个断言检查测试结果的个数是否和测试图片的个数相同。
